        ...
        for resp in self.assistant.Assist(iter_log_assist_requests(), self.deadline):
            ...
            if resp.speech_results:
                ...
                for r in resp.speech_results:
                    commands_split=r.transcript.split(" ")
            ...
            if len(resp.audio_out.audio_data) > 0:
                if not self.conversation_stream.playing:
                    ...
                    if "forward" in commands:
                        ...
                        requests.get("http://localhost/forward.php?time=0.5")
                    elif "backward" in commands:
                        has_command=True
                        print("command detected!")
                        requests.get("http://localhost/backward.php?time=0.5")
                    elif "left" in commands:
                        has_command=True
                        print("command detected!")
                        requests.get("http://localhost/spin.php?angle=-10")
                    elif "right" in commands:
                        has_command=True
                        print("command detected!")
                        requests.get("http://localhost/spin.php?angle=10")
                    elif "spin" in commands:
                        has_command=True
                        print("command detected!")
                        requests.get("http://localhost/spin.php?angle=180")
                        requests.get("http://localhost/spin.php?angle=-180")
                    else:
                        has_command=False
                if not has_command:
